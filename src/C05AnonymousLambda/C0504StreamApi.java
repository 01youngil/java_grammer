...
//
////        원시자료형(int 등)을 가지고 stream객체를 만들게 되면, Stream<T>가 아니라, 별도의 Stream객체 존재
////        int -> IntStream, List<Integer> -> Stream<Integer>
...
//
//        String[] stArr = {"HTML","CSS","JAVA","PYTHON"};
////        제네릭 타입소거로인해, toArray를 통해 바로 새로운 String배열을 만드는 것이 불가
...
////        String[] newStArr = Arrays.stream(stArr).sorted().toArray(a->new String[a]); // 기본 오름차순
...
////        실습1. arr1에서 홀수만 담은 배열을 arr2라는 이름으로 변환
//        int[] arr2 = Arrays.stream(arr1).filter(a->a%2!=0).toArray();
//
////        실습2. arr3에 홀수만 걸러서 해당 홀수의 제곱값을 담기
//        int[] arr3 = Arrays.stream(arr1).filter(a->a%2!=0).map(a->a*a).toArray();
//
////        실습3. arr4에 홀수만 거르고, 제곱값을 구하고, 해당 자들을 오름차순으로 담아주세요.
...
////        optional객체 : 값이 있을수도 있고, 없을수도 있음을 명시한객체
...
////        방법4. orElseThorw : 값이 있으면 있는 값 return, 없으면 지정된 예외 강제발생
